...
// households/{codice}/wasteLog/{id}             -> prodotti scaduti eliminati (per le statistiche)
...
function wasteLogRef(code) {
  return collection(db, 'households', code, 'wasteLog');
}
...
export async function logWaste(code, data) {
  await addDoc(wasteLogRef(code), { ...data, deletedAt: serverTimestamp() });
}

export function subscribeWasteLog(code, onData) {
  const q = query(wasteLogRef(code), orderBy('deletedAt', 'desc'));
